...
import {
	...
	createNewMintKeys,
	verifyProof,
} from "@cashu/crypto/modules/mint";
import { keysetsTable, settingsTable } from "@mnt/common/db";
import type {
	BlindedMessage,
	InsertBlindedMessage,
	InsertMeltQuote,
	InsertMintQuote,
	MeltQuote,
	MintQuote,
	Setting,
} from "@mnt/common/db/types";
...
export class CashuMint {
	...
	async createKeysFromSeed(
	...
	): Promise<{ pubKey: Uint8Array; privKey: Uint8Array }> {
		...
		const persisted = await persistence.insertSeedKeys({
	...
	async swap(
	...
	): Promise<SerializedBlindSignature[]> {
		...
		const isAmountMatch =
			inputs.reduce((prev, curr) => prev + curr.amount, 0) ===
			outputs.reduce((prev, curr) => prev + curr.amount, 0);
		...
		return await tx.transaction(async ({ db }) => {
			await persistence.insertProofs(
				inputs.map((p) => {
					return {
						...p,
						status: CheckStateEnum.SPENT,
			...
			return messages.map((m) => {
				return { amount: m.amount, C_: m.C_, id: m.id };
			});
		});
	}
	...
	async melt(
		...
		if (!isAmountMatch) {
			throw new MintError(
				111,
				"melt failed amounts do not match: fee_reserve + amount != sum(inputs)",
			);
		}
		...
		await persistence.insertProofs(
			inputs.map((p) => {
				return {
					...p,
					status: CheckStateEnum.PENDING,
					Y: hashToCurve(enc.encode(p.secret)).toHex(true),
					meltId: meltQuote.quote,
				};
			}),
		);

		const { preimage, fee } = await this.lightningInterface.payInvoice(
			meltQuote.request,
			meltQuote.fee_reserve * 1000,
		);
		...
		if (preimage) {
			...
			if (outputs?.length) {
				const keys = await this.getKeys(outputs[0].id);
				const keyAmounts = Object.keys(keys.pubKeys)
			...
			return await tx.transaction(async ({ db }) => {
				...
				await persistence.updateProofStatus(
					inputs.map((p) => p.secret),
					CheckStateEnum.SPENT,
				);
				const updatedQuote = await persistence.updateMeltQuoteState(
					quote,
					MeltQuoteState.PAID,
				);
	...
	async checkToken(
		Ys: string[],
	): Promise<{ Y: string; state: CheckStateEnum }[]> {
		const proofs = await persistence.getProofsByYs(Ys);
		return proofs.map((p) => {
			return { Y: p.Y, state: p.status as CheckStateEnum };
		});
	}
	...
	private async validateProofs(proofs: SerializedProof[]): Promise<void> {
		...
		const containsUnknownKeysetId = proofs.find(
			(p) => !keysetIds.includes(p.id),
		);
		...
		const containsInvalidProof = proofs.find(
			(p) =>
				!verifyProof(
					deserializeProof(p),
					findPrivKeyForAmountFromKeyset(keys, p.id, p.amount),
				),
		);
	...
	private async checkKeysetSettings(
		keysetCheck: KeysetSettingsCheck,
	): Promise<void> {
		...
		for (const id of keysetCheck.meltKSIDs ?? []) {
			const unallowedMeltKs = allKeysets.find(
		...
		for (const id of keysetCheck.mintKSIDs ?? []) {
			const unallowedMintKs = allKeysets.find(
		...
		for (const id of keysetCheck.swapOutKSIDs ?? []) {
			const unallowedSwapOutKs = allKeysets.find(
		...
		for (const id of keysetCheck.swapInKSIDs ?? []) {
			const unallowedSwapInKs = allKeysets.find(
	...
	private async checkMintSettings(amount: number) {
		...
		const maxMintAmt = Number.parseInt(
			allSettings.find((s) => s.key === "mint-max-amt")?.value ?? "0",
		);
		if (maxMintAmt && maxMintAmt < amount) {
			throw new MintError(
				101,
				`Mint amount exceeds max allowed amount: tried = ${amount} , allowed = ${maxMintAmt}`,
			);
		}
		const minMintAmt = Number.parseInt(
			allSettings.find((s) => s.key === "mint-min-amt")?.value ?? "0",
		);
		if (minMintAmt && minMintAmt > amount) {
			throw new MintError(
				101,
				`Mint amount is lower than min allowed amount: tried = ${amount} , allowed = ${minMintAmt}`,
			);
		}
	}
	private async checkMeltSettings(amount: number) {
		...
		const maxMeltAmt = Number.parseInt(
			allSettings.find((s) => s.key === "melt-max-amt")?.value ?? "0",
		);
		if (maxMeltAmt && maxMeltAmt < amount) {
			throw new MintError(
				101,
				`Melt amount exceeds max allowed amount: tried = ${amount} , allowed = ${maxMeltAmt}`,
			);
		}
		const minMintAmt = Number.parseInt(
			allSettings.find((s) => s.key === "melt-min-amt")?.value ?? "0",
		);
		if (minMintAmt && minMintAmt > amount) {
			throw new MintError(
				101,
				`Melt amount is lower than min allowed amount: tried = ${amount} , allowed = ${minMintAmt}`,
			);
		}
	}
}
