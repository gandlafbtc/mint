...
export class MintPersistenceImpl {
	async insertSeedKeys(
		keys: { pubKey: string; privKey: string },
	...
	async getProofsByState(state: CheckStateEnum): Promise<Proof[]> {
		return await db
			.select()
			.from(proofsTable)
			.where(eq(proofsTable.status, state));
	}
	...
	async addKeyset(
		...
		await db
			.update(keysetsTable)
			.set({ isActive: false })
			.where(eq(keysetsTable.isActive, true));
		...
		for (const pubK of Object.entries(keysetPair.pubKeys)) {
			const amount = Number.parseInt(pubK[0]);
			keys.push({
				amount,
				keysetHash: keysetPair.keysetId,
				pubKey: bytesToHex(pubK[1]),
				secKey: bytesToHex(keysetPair.privKeys[amount]),
	...
	async getKeysets(tx?: BunSQLiteDatabase): Promise<SerializedKeyset[]> {
		...
		const keysets: SerializedKeyset[] = keysetsFromDb.map((k) => {
			return { active: k.isActive ?? false, id: k.hash, unit: k.unit ?? "sat" };
		});
		return keysets;
	}

	async createMintQuote(
		quote: InsertMintQuote,
		tx?: BunSQLiteDatabase,
	): Promise<MintQuote> {
		const db = tx ?? database;
		const insertedQuote = (
			await db.insert(mintQuotesTable).values(quote).returning()
		)?.[0];
		eventEmitter.emit("socket-event", {
			command: "inserted-mint-quote",
			data: { quote: insertedQuote },
		});
		return insertedQuote;
	}

	async updateMintQuote(
		quote: MintQuote,
		tx?: BunSQLiteDatabase,
	): Promise<MintQuote> {
		const db = tx ?? database;
		await db
			.update(mintQuotesTable)
			.set(quote)
			.where(eq(mintQuotesTable.quote, quote.quote));
		return quote;
	}
	async updateMintQuoteState(
		...
		await db
			.update(mintQuotesTable)
			.set({ state: state })
			.where(eq(mintQuotesTable.quote, quote));
	}
	async getMintQuote(
		quote: string,
		tx?: BunSQLiteDatabase,
	...
	async insertMessages(
		messages: InsertBlindedMessage[],
		tx?: BunSQLiteDatabase,
	) {
		const db = tx ?? database;
		const insertedMessages = await db
			.insert(blindedMessagesTable)
			.values(messages)
			.returning();
		eventEmitter.emit("socket-event", {
			command: "inserted-messages",
			data: { messages: insertedMessages },
		});
	...
	async insertProofs(proofs: InsertProof[], tx?: BunSQLiteDatabase) {
		const db = tx ?? database;
		const insertedProofs = await db
			.insert(proofsTable)
			.values(proofs)
			.returning();
		eventEmitter.emit("socket-event", {
			command: "inserted-proofs",
			data: { proofs: insertedProofs },
		});
		return insertedProofs;
	}
	async updateProofStatus(
		secrets: string[],
		status: CheckStateEnum,
		tx?: BunSQLiteDatabase,
	) {
		const db = tx ?? database;
		return await db
			.update(proofsTable)
			.set({ status: status })
			.where(inArray(proofsTable.secret, secrets));
	}
	...
	async getMeltQuote(
		...
		const quotes = await db
			.select()
			.from(meltQuotesTable)
			.where(eq(meltQuotesTable.quote, quote));
		if (!quotes.length) {
			throw new Error("No such mint quote");
		}
		return quotes[0];
	}
	async modifyKeysetStatus(
		keysetId: string,
		isEnabled: boolean,
		tx?: BunSQLiteDatabase,
	): Promise<Keyset> {
	...
	async getProofsByYs(Ys: string[], tx?: BunSQLiteDatabase): Promise<Proof[]> {
		...
		const proofs = await db
			.select()
			.from(proofsTable)
			.where(inArray(proofsTable.Y, Ys));
		return proofs;
	}
	...
	async getBMsByB_(
		...
		const bms = await db
			.select()
			.from(blindedMessagesTable)
			.where(inArray(blindedMessagesTable.B_, B_s));
		return bms;
	}
	...
	async getChange(
		...
		const bms = await db
			.select()
			.from(blindedMessagesTable)
			.where(eq(blindedMessagesTable.changeId, quote));
		return bms;
	}
	...
	async createMeltQuote(
		quote: InsertMeltQuote,
		tx?: BunSQLiteDatabase,
	): Promise<MeltQuote> {
		const db = tx ?? database;
		const insertedQuote = (
			await db.insert(meltQuotesTable).values(quote).returning()
		)?.[0];
		eventEmitter.emit("socket-event", {
			command: "inserted-melt-quote",
			data: { quote: insertedQuote },
		});
		return insertedQuote;
	}
