...
export const checkPendingProofs = async () => {
	...
	for (const q of meltquotes) {
		...
		if (invoice?.state === "FAILED" || invoice?.state === "CANCELED") {
			...
			await db.delete(proofsTable).where(
				inArray(
					proofsTable.uid,
					failedProofs.map((p) => p.uid),
				),
			);
