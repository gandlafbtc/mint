...
export class LNDBackend implements Lightning {
	...
	async getBalance(): Promise<{
		lnBalance: ChannelBalanceResponse;
	...
	async getNewInvoice(
		amount: number,
	): Promise<{ paymentRequest: string; rHash: string | Buffer | Uint8Array }> {
		const lnd = await LND.getInstance();
	...
	async getInvoice(
		hash: string,
	): Promise<{ paymentRequest: string; state: string }> {
		const lnd = await LND.getInstance();
		const hashUint = hexToBytes(hash);
		const invoice = await lnd.invoices.lookupInvoiceV2({
			paymentHash: hashUint,
		});
		return invoice;
	}

	async getInvoiceByInvoice(
		invoice: string,
	): Promise<{ paymentRequest: string; state: string }> {
		const lnd = await LND.getInstance();
		const decoded = await lnd.lightning.decodePayReq(
			invoice as PayReqStringPartial,
		);
	...
	async estimateFee(request: string): Promise<{ fee: number }> {
		...
		try {
			const res = await lnd.router.estimateRouteFee({
				timeout: 30,
				paymentRequest: request,
			});
			if (res.failureReason !== "FAILURE_REASON_NONE") {
				const { numSatoshis } = await lnd.lightning.decodePayReq({
					payReq: request,
				});
				const amount = Number.parseInt(numSatoshis);
				const fee =
					settings.feeEstBase +
					Math.ceil((settings.feeEstPercent / 100) * amount);
				return { fee };
			}
			return { fee: Math.ceil(Number.parseInt(res.routingFeeMsat) / 1000) };
		} catch (error) {
			const { numSatoshis } = await lnd.lightning.decodePayReq({
				payReq: request,
			});
	...
	async payInvoice(
		request: string,
		feeLimitMsat: number,
	): Promise<{ preimage: string; fee: number }> {
		const lnd = await LND.getInstance();
		const stream = lnd.router.sendPaymentV2({
				...
				switch (payment.status) {
					...
					case "SUCCEEDED":
						...
						resolve({
							...
							fee: Number.parseInt(payment.feeSat),
						});
						break;
					case "FAILED":
					case "UNKNOWN":
						...
						reject(
							new Error(
								`${payment.paymentHash} Payment failed: ${payment.failureReason}`,
							),
						);
						break;
				}
	...
	async getInvoiceAmount(request: string): Promise<{ amount: number }> {
		...
		return { amount: Number.parseInt(numSatoshis) };
	}
}
